...
- Le nom de relation (`name`) est un **accesseur côté source** : il doit être unique **par entité source** (`from`), pas globalement.
  `Classe.annee_scolaire` et `InscriptionEleve.annee_scolaire` coexistent.
- La clé étrangère (`foreign_key`) est une **colonne de la table source** : elle doit être unique **par table source**.
  `classe.annee_scolaire_id` et `inscription_eleve.annee_scolaire_id` sont deux colonnes distinctes.
