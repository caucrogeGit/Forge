...
| `forge-mvc-sessions-db` | `4 - Beta` | Store de session persistant opt-in extrait du cœur (ADR-054) : `DbSessionStore`, sessions dans la table `forge_sessions` partagées entre workers. SQL portable (horodatages Python, sans `NOW()` propriétaire) adossé au backend BDD actif via `core.database.db`. Dépend uniquement du cœur. Non publié sur PyPI |
| `forge-mvc-fixtures` | `4 - Beta` | Données de démo et de test opt-in (ADR-074) : commandes `fixtures:load`/`fixtures:purge` pour des jeux de données rejouables et cadrés par environnement, à SQL visible. Opt-in CLI-only, sans API runtime ni migration ; frontière nette avec la migration de seed. Dépend uniquement du cœur. Scaffold en cours, non publié sur PyPI |
...
| `forge-mvc-postgres` | `4 - Beta` | Backend BDD PostgreSQL (ADR-054) au-dessus de `psycopg` : dialecte + traduction des paramètres `?` vers `%s`. Niveau plein (promotion ADR-084 du 2026-07-19) : provisioning `db:init` et intégration (couche DB, migrations, introspection) validés en CI contre un vrai serveur. Publié sur PyPI |
| `forge-mvc-mssql` | `4 - Beta` | Backend BDD Microsoft SQL Server (ADR-054) au-dessus de `pyodbc` : dialecte Transact-SQL (IDENTITY, crochets, formes gardées IF OBJECT_ID) ; pyodbc utilise nativement les paramètres `?`, pilote ODBC système requis. Niveau plein (promotion ADR-084 du 2026-07-19) : provisioning `db:init` et intégration validés en CI contre un vrai serveur. Publié sur PyPI |
