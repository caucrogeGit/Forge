...
| FND-DOC-004 | Documentation | Références `v3.0.1` dans guides d'installation principaux (3 occurrences : `git clone --branch v3.0.1`) | IMPORTANT | OUVERT | VERSION-COHERENCE-1.0-001 | §3 audit renforcé. Les occurrences `v3.0.1` dans les guides d'installation ont été remplacées par `{{forge_tag}}` (variable de template). Les mentions de type `3.1+` et `3.1.0` dans les docs publiques actives ont été corrigées vers la trajectoire `1.0.x`. Constat partiellement traité — vérification de complétude à confirmer lors de la validation administrative. |
...
| ID | Domaine | Constat | Gravité | Statut | Ticket cible | Notes |
|---|---|---|---|---|---|---|
| FND-SESSION-001 | Sessions | Sessions in-memory bloquantes en production multi-process — sessions non partagées entre workers | IMPORTANT | OUVERT | SESSIONS-CONFIGURABLE-STORE-001 | §5.5 audit Claude |
...
| FND-MODULE-002 | Modules | Injection de routes modules via écriture dans `mvc/routes.py` — violation du principe §9 | IMPORTANT | OUVERT | MODULE-ROUTES-INJECTION-REMOVE-001 | §7.3 audit Claude |
