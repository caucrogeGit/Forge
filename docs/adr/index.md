...
| [ADR-080](080-auth-subject-validation.md) | Validation du **sujet** authentifié : `user_loader` d'auth optionnel et enregistrable (`core.auth.register_user_loader`) ; `is_authenticated(request)` devient adossé au sujet (existe et actif) quand un loader est enregistré, id-based sinon (rétrocompat). `AuthMiddleware` valide le sujet et ferme la session orpheline (logout + purge cookie) avant de rediriger (F54) ; contexte de rendu unifié (F55). Une seule définition (principe 11), cohérente avec `can()`/`current_user` ; retour terrain 021 (proposée) |
