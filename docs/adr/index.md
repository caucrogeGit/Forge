...
| [ADR-071](071-optin-db-provisioning-convention.md) | Convention unique de provisioning des opt-ins adossés à la base : la commande `<opt-in>:init` dépose une migration dans `mvc/migrations/`, appliquée par `forge migration:apply` (déjà suivie par 7 opt-ins) ; `mvc/models/sql/` + `db:apply` reste réservé au modèle applicatif (socle auth) ; `forge-mvc-sessions-db` est réaligné (proposée) |
