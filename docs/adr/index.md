...
| [ADR-077](077-fixtures-reliees.md) | Fixtures **reliées** : `make-factory` scaffolde les **colonnes réelles** de l'entité (F45, via `column_for_field` d'entities), `Factory.reference(table, col, valeur)` rendu en **sous-requête SQL** `(SELECT Id FROM … WHERE … LIMIT 1)` pour lier une fixture à l'Id créé par une autre (F43), et `fixtures:load` **trie topologiquement** par dépendances FK depuis `relations.json` (F44, repli nom de fichier, option `--no-fk-checks`) ; API additive, dépendance douce à entities ; piste callable différée (proposée) |
