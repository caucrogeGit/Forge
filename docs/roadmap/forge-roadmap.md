...
| 4. Typage | `TYPING-CLI-STRICT-001` | **Fait (mode standard)** : 25 erreurs pyright de `cli/` corrigées (dont un **vrai bug runtime** : `core.database.fetch_one` inexistant dans l'auth admin) ; `cli/` gaté en CI par le meta-test `test_cli_pyright_clean_001` (`pyright cli` = 0 erreur), sans toucher au pyproject protégé. **Reste** : mode strict complet (en-têtes `# pyright: strict` par fichier + `TypedDict` du schéma d'entité) | partiel |
