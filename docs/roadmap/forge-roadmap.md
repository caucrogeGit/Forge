...
| `OPTIN-RUNTIME-PAGINATION-DIALECT-001` | **ouvert** | Cinq opt-ins écrivent leur pagination en syntaxe MySQL dans le SQL qu'ils exécutent, donc hors des générateurs : `forge-mvc-admin` (`query.py:55`, `LIMIT ? OFFSET ?`), `forge-mvc-audit` (`store.py:120`), `forge-mvc-iot` (`storage/repository.py:68,76`), `forge-mvc-mail` (`log.py:26`) et `forge-mvc-notifications` (`store.py:113`). T-SQL ne connaissant pas `LIMIT`, ces lectures échouent sur SQL Server, promu au niveau plein par l'ADR-084. Même famille que `ENTITIES-CRUD-PAGINATION-DIALECT-001`, mais **le trait posé par ce dernier ne suffit pas** : quatre de ces cas paginent avec `LIMIT ?` sans décalage, forme qui demande son propre rendu (`SELECT TOP (?)` ou `OFFSET 0 ROWS FETCH NEXT ? ROWS ONLY` en T-SQL). Demande donc une décision de contrat avant implémentation. Sévérité Élevée : casse à l'exécution, pas seulement à la génération. |
