...
| `GOV-SESSIONS-LEAK-CLOSE-001` | **ouvert** | Traces laissées par l'extraction ci-dessus : les deux pages `official-site/docs/forge/deployment/{deployment,mise-en-production}.md` importent encore `core.sessions.mariadb_store`, **module supprimé** (un lecteur qui les suit obtient un `ImportError`) ; `docs/release/release-policy.md` cite `MariaDbSessionStore` ; `core/database/connection.py` annonce un « repli MariaDB intégré » qui n'existe plus. Sévérité Moyenne : documentation publiée trompeuse, aucun effet runtime. |
...
| `OPTIN-SQL-TYPE-BRANCHING-AUDIT-001` | **audit livré** | Défaut voisin mais distinct, découvert en élargissant le scan du garde-fou précédent : cinq emplacements **décident d'un comportement à partir d'un nom de type SQL** (préfixes `VARCHAR`, `TEXT`, `LONGTEXT`...). Plus sournois qu'un DDL non portable, qui échoue franchement : ici la condition est simplement fausse et la fonctionnalité disparaît **en silence**. Mesuré : sur SQL Server, dont les types commencent par `NVARCHAR`, le CRUD généré perd **toute** sa recherche `LIKE`, ses `textarea` et ses libellés de relation ; sur SQLite les filtres de liste sont refusés. Le contrat expose pourtant déjà `sql_families()`. Rapport complet : [`docs/history/audits/optin-sql-type-branching-audit-001.md`](../history/audits/optin-sql-type-branching-audit-001.md). Arbitrage requis entre corriger par `sql_families()` (local) et décider à partir du type Forge (cause). |
