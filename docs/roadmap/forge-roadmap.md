...
| `OPTIN-DDL-DIALECT-AUDIT-001` | **audit livré** | Mesuré sur quatre serveurs réels : **12 fichiers SQL livrés par 10 opt-ins, aucun portable sur les quatre moteurs** (MariaDB 11/12, SQLite 0/12, PostgreSQL 0/12, SQL Server 0/12). Ces opt-ins écrivent du SQL MariaDB à la main au lieu de passer par le contrat `Dialect`, qui couvre pourtant déjà les cinq constructions en cause. Quatre d'entre eux définissent en outre la même table deux fois (migration `.sql` et constante Python), contre le principe 11. Rapport complet : [`docs/history/audits/optin-ddl-dialect-audit-001.md`](../history/audits/optin-ddl-dialect-audit-001.md). Arbitrage requis entre rendre le DDL dialectal et assumer la restriction MariaDB. |
