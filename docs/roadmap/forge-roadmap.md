...
Faible : 5 livrés (`TEST-META-CORE-NO-OPTIN-IMPORT-001`, `DOC-U2014-ROADMAP-RESIDUAL-001`, `CI-DEPAUDIT-DOC-LINK-001`, `SEC-MFA-DECRYPT-MSG-001`, `TEST-STRICT-MARKERS-001`) ; `REFACTOR-BUILDERS-DECOMPOSE-002` : `build_controller` décomposé (317 vers 135 lignes), `build_model`/`build_form_view`/`build_table_partial` à suivre ; `GOV-CLAUDE-MD-OPTINS-RESYNC-001` reste au mainteneur (fichier protégé, au prochain tag).
...
| `REFACTOR-BUILDERS-DECOMPOSE-002` | Finir la décomposition des builders CRUD : extraire `_render_list_context` + `_render_preamble` de `build_controller` (reste 317 l.), puis traiter `build_model` (254 l.), `build_form_view` (212 l.), `build_table_partial` (159 l.) sur le même modèle. Iso-sortie garantie par baseline | ⚪ ◑ build_controller fait |
