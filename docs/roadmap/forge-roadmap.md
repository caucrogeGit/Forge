...
> Version courante : **Forge 1.0.0-rc.9** (release candidate, préparée le 2026-09-08).
> Dernière version publiée sur PyPI : **Forge 1.0.0-rc.8** (2026-09-07), 28 distributions.
> Vingt-huit tickets sont livrés depuis ce tag, tous consignés au `CHANGELOG.md`.
> Un contrôle de pré-vol refuse de publier un journal qui tairait un ticket livré.

---

## État actuel : Forge 1.0.0-rc.9

**Tag courant : `v1.0.0-rc.9`**, huitième release candidate avant la 1.0.0 stable.

Elle ne contient que des correctifs.
Vingt-huit tickets, nés de deux revues externes de la rc8, l'une sur le cœur et l'autre sur les vingt-sept opt-ins.

Les vingt-deux défauts qu'elles signalaient ont été **reproduits sur le dépôt avant d'être corrigés**, et non pris pour argent comptant.
La seconde revue fournissait dix-sept sondes exécutables : les dix-sept se confirmaient, et les dix-sept sont fermées.

Un seul motif les explique presque toutes.

**Un contrôle qui existe, qui passe, et qui regarde à côté.**
Aucun de ces défauts ne venait d'une garde absente.
Le contrôle anti-injection d'en-têtes lisait le dictionnaire applicatif, quand les deux serveurs y ajoutaient ensuite le type de contenu et les cookies.
Une condition de transition était cherchée sous une casse que l'appelant n'avait pas.
Le plan de purge des fixtures lisait le contenu d'une chaîne SQL comme du code.
Le garde de démarrage IoT ignorait le mode d'authentification que son propre contrôleur savait traiter.

**Un instrument de mesure faux ne rend pas les tests inutiles, il les rend rassurants.**
Le client de test n'encodait pas les chemins comme un serveur, si bien qu'il **masquait** le défaut de décodage d'URL du cœur : un client qui n'imite pas un serveur ne peut pas révéler qu'un serveur se trompe.
Il a donc été corrigé en premier, avant des défauts pourtant plus graves.
Le contrôle de complétude du journal, posé la veille, comptait pour sa part deux tickets sur vingt.

**Trois correctifs de sécurité.**
Le type de contenu et les cookies échappaient au contrôle anti-injection sur les deux chemins de sortie.
Un code TOTP accepté pouvait être consommé trois fois, la tolérance de plus ou moins un pas se combinant à un anti-rejeu fondé sur l'horloge, là où la RFC 6238 section 5.2 demande qu'un code accepté ne soit pas rejouable.
La connexion SQL Server ne vérifiait jamais le certificat du serveur.

**Une garantie tient du début à la fin d'une opération, ou elle ne tient pas.**
Un ouvrier dont le bail avait expiré terminait la tâche d'un autre.
Une reprise de traitement vidéo recomptait sa propre durée dans le quota.
Une suppression de fichier aboutissait, puis levait, sans nettoyer le registre.
Une catégorie de statistiques ne se retrouvait pas par son propre filtre.

L'API publique reste gelée pour la 1.0 et tous les opt-ins restent en Beta (`Development Status :: 4 - Beta`).

Un point demande un geste côté exploitant : un projet SQL Server sur certificat auto-signé doit désormais déclarer `DB_MSSQL_TRUST_SERVER_CERTIFICATE=yes`.
C'est le seul des vingt-huit correctifs dans ce cas, et la référence du paquet le documente.

Précédent : v1.0.0-rc.8 (2026-09-07, pré-mortem des vingt-sept opt-ins, application désarmée en production ADR-092 et ADR-093), v1.0.0-rc.7 (2026-08-17, pré-mortem du cœur et de chaque opt-in, nonce CSP inerte en production), v1.0.0-rc.6 (2026-08-14), v1.0.0-rc.4 (2026-08-05, vérification de la documentation par exécution, correctif `GET /health` sous WSGI), v1.0.0-rc.3 (2026-07-31, correctif CRLF des en-têtes, DDL dialectale des opt-ins), v1.0.0-rc.2 (2026-07-01, refonte de la navigation documentaire, dispatch CLI par entry points ADR-059), v1.0.0-rc.1 (2026-06-26, première release candidate, briques opt-in ADR-052, déploiement extrait ADR-053), v1.0.0-beta.17 (2026-06-18, typage strict de bout en bout ADR-036), v1.0.0-beta.16 (2026-06-16, retrait de la génération de starters ADR-035), v1.0.0-beta.15 (2026-06-08, i18n opt-in ADR-027, convention de route ADR-029, refonte welcome-forge ADR-025/028), v1.0.0-beta.14 (2026-06-07, squelette dédié ADR-024), v1.0.0-beta.13 (2026-06-06), v1.0.0-beta.12 (2026-05-29), v1.0.0-beta.9 (2026-05-24), v1.0.0-beta.8 (2026-05-22), v1.0.0-beta.7 (2026-05-22), v1.0.0-beta.6 (2026-05-21), v1.0.0-beta.5 (2026-05-17), v1.0.0-beta.3 (2026-05-16), v1.0.0-beta.2 (2026-05-16), v1.0.0-beta.1 (2026-05-15), v3.0.5 (2026-05-14), v3.0.4 (2026-05-14), v3.0.3 (2026-05-14), v3.0.2 (2026-05-13), v3.0.1 (2026-05-12), v3.0.0 (2026-05-12).

**Statut : v1.0.0-rc.9, huitième release candidate : API publique gelée, tous les opt-ins en Beta, vingt-huit correctifs issus de deux revues externes intégralement reproduites avant correction.
...
| 2. Onboarding | `DOC-CRUD-WORKFLOW-CANONICAL-001` | **Fait** : workflow officiel arrêté = `make:entity → build:model → db:init → make:crud` (`build:model` régénère SQL + `_base.py` + `relations.sql` en une commande ; `sync:entity`/`sync:relations` = régénération ciblée). Plus de contradiction : le tutoriel rejoint `guide.md` (déjà sur `build:model`) | fait |
| 2. Onboarding | `DOC-ONBOARDING-TUTORIAL-FIX-001` | **Fait** : `app-complete-tutorial.md` réécrit (JSON canonique clé `name` + types Forge, dossiers snake_case, `build:model`, lancement `forge run`, bandeau « legacy » retiré) ; URL `http`→`https` de `poste-linux.md` corrigée ; garde-fou `test_doc_app_complete` MAJ | fait |
| 3. Release | `PKG-VERSION-SYNC-CHECK-001` | **Fait** : `tools/check_version_sync.py` dérive la version canonique (pyproject racine) et vérifie tous les emplacements (14 sous-paquets, pins `forge-mvc`, extras, squelette, `core/__init__`, `forge.py`, `package.json` en SemVer). Câblé dans les deux scripts release et dans un meta-test (`test_version_sync_001`) exécuté à chaque push | fait |
| 3. Release | `PKG-OPTIN-LICENSE-FILES-001` | **Faux positif (vérifié par build)** : le LICENSE est déjà embarqué dans les wheels via le glob `license-files` par défaut de setuptools >= 77 (`…dist-info/licenses/LICENSE` présent). Aucune action : le projet s'appuie volontairement sur le glob par défaut (documenté dans `test_optin_license_present_001`) | clôturé |
| 4. Typage | `TYPING-CLI-STRICT-001` | **Fait (mode standard)** : 25 erreurs pyright de `cli/` corrigées (dont un **vrai bug runtime** : `core.database.fetch_one` inexistant dans l'auth admin) ; `cli/` gaté en CI par le meta-test `test_cli_pyright_clean_001` (`pyright cli` = 0 erreur), sans toucher au pyproject protégé. **Strict en cours** : ratchet par fichier établi (le gate enforce `# pyright: strict` dès qu'un fichier le porte), 4 fichiers passés en strict. **Reste** : ~1695 erreurs strict mesurées sur 38 fichiers, à résorber par fichier ; deux cascades à traiter (TypedDict du schéma d'entité pour les gros générateurs `controller_builder`/`make_entity`/`views_builder` ; renommage public des fonctions privées utilisées inter-modules, ex. `_with_permission`, `_ensure_route`) | en cours |
...
| 1 | `TYPING-CLI-CRUD-001` | **Fait** : tout `cli/entities/crud/` strict (utils, context, form/model/views/controller_builder, relations_loader). Hub `utils.py` typé d'abord (haut-levier), puis schéma d'entité `dict[str, Any]` + `cast` aux `or {}`/itérations, dataclass `default_factory` typé, `reportPrivateUsage=false`/`reportUnusedFunction=false` pour le partage intra-paquet | ✅ |
| 2 | `TYPING-CLI-ENTITIES-VALIDATION-001` | **Fait** : `validation.py`, `entity_validate.py`, `entity_semantic_validate.py`, `entity_validation_errors.py` strict. Motif : caster une fois chaque variable après son guard `isinstance` (data/field/entity/media/relation/pivot), schéma `dict[str, Any]`, frontière jsonschema typée `Any` (réutilise la solution security), probes `import jsonschema as jsonschema` | ✅ |
| 3 | `TYPING-CLI-ENTITIES-MAKE-DB-001` | **Fait** : `make_entity`, `make_crud`, `relations`, `migrations`, `db_init`, `db_apply`, `model` strict. Motif DB : params `cursor`/`connection`/`db` typés `Any`, `import mariadb` ignoré (pas de stubs), `mariadb.connect` casté ; make_crud = hub de re-exports (`reportUnusedImport=false`) ; cast après guards isinstance. **Garde-fou** : la regex de typage avait sur-corrigé 3 chaînes-templates de code généré (`to_dict`/`from_dict`/`_form_data_from_*`), revertées (le code généré garde `dict`/`-> dict`) | ✅ |
...
| `DOC-ONBOARDING-DB-INIT-001` | **Fait** : les 3 paliers SQL welcome (`first-sql`, `list-records`, `relations`) introduisent `forge db:init` (une fois/mini-projet, ADR-028) puis `forge migration:make <nom>` avant `forge migration:apply`. Le parcours doc→app SQL qui tourne n'est plus cassé | ✅ |
| `AUTH-LOG-SWALLOWED-EXC-001` | **Fait** : les 4 `except Exception` larges de `core/auth/session.py` journalisent en WARNING avec trace (`exc_info`) avant `return None` ; un loader qui lève (infra) est désormais visible et distinct d'un échec d'auth normal (non journalisé). Test caplog dédié (`test_auth_log_swallowed_exc_001`) | ✅ |
...
| `SEC-MULTIWORKER-STATE-001` | **Fait (version doc)** : la doc de déploiement couvrait déjà sessions + rate-limits login/upload en multi-worker ; ajout du **seul point manquant**, l'anti-rejeu TOTP/MFA (code rejouable sur un autre worker), dans `production-limits.md` (§2/§4/§7) et `wsgi-deployment.md`. Le store partagé distribué reste un éventuel ticket futur (déjà listé en « Non garanti ») | ✅ |
| `DOC-STYLE-U2014-CLEANUP-001` | **Fait** : ~4400 tirets cadratins U+2014 remplacés par la virgule (`README.md` à 0, 0 cadratin de prose dans tout `docs/`), sweep fence/inline-code-aware (blocs de code et `reference/markdown.md` qui documente `-` préservés), `docs/history/` exclu (mémoire brute §9D), 2 guards doc-as-test roadmap mis à jour | ✅ |
| `SEC-RBAC-MISSING-TABLE-DETECT-001` | **Fait** : `_is_missing_table_error` détecte d'abord par `errno == 1146` (ER_NO_SUCH_TABLE, robuste locale/version), string-matching en repli seulement. Test : message localisé + errno=1146 reconnu | ✅ |
| `SEC-RBAC-CANONICAL-GUARD-001` | **Fait** : `require_user_permission` (DB, autoritatif) désignée canonique au niveau **code** (docstring) ; caveat sécurité ajouté à `require_permission` (vérifie les permissions request/session, pas la base, peut être périmé) ; incohérence doc corrigée (permissions fines -> `require_user_permission`). La doc désignait déjà la garde DB pour les nouveaux projets | ✅ |
| `SEC-JINJA-AUTOESCAPE-001` | **Fait** : `select_autoescape(default_for_string=True, default=True)` : le renderer de vues cœur échappe désormais TOUTES les extensions + chaînes (plus seulement `.html`). Mail (renderer indépendant) inchangé. Test behavior-first (.txt/.svg/sans-ext échappés, `|safe` toujours possible) | ✅ |
| `ADMIN-RBAC-FAILCLOSED-001` | **Fait** : `_permission_guard` passe en fail-**closed** : une route admin avec permission déclarée mais `forge-mvc-rbac` absent renvoie **403 + WARNING loggué** (au lieu de laisser passer silencieusement). Sans permission déclarée, l'admin reste accessible (auth seule). Test fail-closed | ✅ |
| `CI-AUDIT-BLOCKING-001` | **Fait (pip-audit + bandit)** : `pip-audit --ignore-vuln PYSEC-2026-217 -r requirements.txt` bloquant dans `tests.yml` (via le quality gate), + `bandit` SAST informatif (non bloquant). SBOM CycloneDX à la release = follow-up (workflow deploy) | ✅ |
| `CI-PYRIGHT-ORDER-001` | **Fait** : pip-audit/ruff/pyright passés en `continue-on-error` (avec `id`) ; pytest + builds tournent toujours ; un **Quality gate** final (`if: always`) fait échouer le job si l'un a échoué. Plus aucun masquage de la suite (piège ADR-044 réglé) | ✅ |
| `CLI-MODULE-REMOVE-ROUTE-001` | **Fait (option A)** : `module:remove` routé dans `forge.py` (le code + la doc « Disponible » + le docstring « CLI dispatche » confirmaient l'intention publique ; les guards « gelés » étaient des snapshots de l'état bugué). Ajouté au help, à `cli-commands.md`, et les 3 snapshot guards mis à jour (famille=5, liste auditée, doc-coverage) + test de dispatch via `forge.main()` | ✅ |
| `GOV-DOC-COHERENCE-001` | **Partie ADR faite** : ADR-018/019/020 passés « Proposé » → « Acceptée » (extractions `forge-mvc-images`/`forge-mvc-files` livrées). **Reste, fichier protégé (prochain tag)** : CLAUDE.md §9 « 12 packages » → 14 (admin/testing), à faire à la main par le mainteneur (hook bloque l'édition agent). Complète `DOC-CHARTE-REALIGN-AUDITABLE-001` | 🟡 partiel |
| `TYPING-DEAD-DIRECTIVES-001` | **Fait** : 10 directives `reportPrivateUsage=false` mortes retirées (db_apply, validation, entity_validation_errors, model, db_init, entity_validate, entity_semantic_validate, make_entity, relations, crud/context) ; vérifié empiriquement (retrait -> pyright reste 0), les 12 justifiées gardées | ✅ |
| `CLI-SHARED-HELPERS-RENAME-001` | **Fait** : `_split_sql_statements` -> `split_sql_statements` (db_apply), `_sql_default_literal` -> `sql_default_literal` (make_entity) ; ces helpers partagés cross-module avec migrations.py n'ont plus le `_` trompeur, et la directive `reportPrivateUsage=false` de migrations.py (devenue inutile) est retirée | ✅ |
| `REFACTOR-BUILD-CONTROLLER-001` | **Fait** : `build_controller` 824 -> 318 lignes ; les 11 méthodes du contrôleur extraites en sous-générateurs module `_render_*(ctx)` (index, new, create, show, edit, update, destroy, bulk_delete×2, _csv_escape, export_csv) via une dataclass `_ControllerContext`. Iso-sortie garantie (baseline SHA + 2648 tests de génération). Header/imports + list_context restent dans l'orchestrateur (construction conditionnelle du préambule) | ✅ |
...
| `TEST-META-ROTATION-APPLY-001` | **Fait** (1re passe conservatrice) : application de la politique de rotation dans son ticket dédié. 6 marqueurs de clôture redondants retirés, leur invariant (le doc d'audit d'archive existe) restant couvert par un guard frère (critère #2 de la politique) : `test_legacy_close_001` (couvert par `test_legacy_remove_plan_001`), `test_legacy_warnings_close_001` (par `test_legacy_warnings_audit_001`), `test_nullable_close_001` (par `test_nullable_contract_audit_001`), `test_pivot_crud_close_001` (par `test_pivot_crud_audit_001`), `test_rbac_module_close_001` (par `test_rbac_module_package_audit_002`), `test_pivot_advanced_close_001` (par `functional_model_001` + `ux_model_002`). Tous purement documentaires (archives figées `docs/history/audits/`), phases définitivement closes, non-sécurité. Garde-fous frères + `rotation_policy` + `isolate` verts (112), collecte meta OK (6889). Reste : poursuite incrémentale au fil de l'eau, « le doute profite au maintien » | ✅ |
...
Faible : 5 livrés (`TEST-META-CORE-NO-OPTIN-IMPORT-001`, `DOC-U2014-ROADMAP-RESIDUAL-001`, `CI-DEPAUDIT-DOC-LINK-001`, `SEC-MFA-DECRYPT-MSG-001`, `TEST-STRICT-MARKERS-001`) ; `REFACTOR-BUILDERS-DECOMPOSE-002` livré : les 4 builders CRUD décomposés (`build_controller` 317 vers 135, `build_model` 254 vers 100, `build_form_view` 212 vers 50, `build_table_partial` 159 vers 88), iso-sortie garantie par baseline sha256 + tests CRUD ; `GOV-CLAUDE-MD-OPTINS-RESYNC-001` livré (CLAUDE.md resync par le mainteneur via patch : opt-ins 12 vers 15, ADR 031 vers 051, statut ADR-030).

| Ticket | Responsabilité unique | Sévérité |
|---|---|---|
| `QRCODE-TYPING-SEGNO-STUBS-001` | `segno` **expose des stubs** (`py.typed`, `__init__.pyi`) : retirer les contournements `cast("Any", segno)`, `# pyright: ignore[reportMissingTypeStubs]` et `qr: Any` de `generator.py:12,14,31,45`, typer `qr: segno.QRCode`. Typage strict authentique (le `# pyright: strict` masque aujourd'hui des `Any`) | 🟠 ✅ fait |
...
| `QRCODE-ERROR-CONTRACT-001` | Compléter le contrat d'erreurs de `forge-mvc-qrcode` (principe 10) : `from_text`/`to_png`/`to_svg`/`QrCodeResponse` laissent remonter des exceptions **segno brutes** sur `error` invalide, overflow (>capacité QR), `scale`/`border` négatifs. Valider `error ∈ {l,m,q,h}`, borner `scale≥1`/`border≥0`, mapper en `QrCodeError` ; ajouter les tests edge-cases manquants | 🟠 ✅ fait |
| `TEST-DB-INIT-MIGRATIONS-001` | Ajouter des tests d'intégration MariaDB (`-m db`, `tests/db/`) pour `db:init` (création comptes/GRANT, mode dégradé `CREATE USER IF NOT EXISTS`) et le **runner de migrations** (apply + rollback réels). Aujourd'hui les 25 tests `db` couvrent `make_entity`/`db:apply`/transactions/FK/slug mais pas ces deux chemins (ADR-033/034), validés en mock seulement | 🟠 ✅ fait |
| `DOC-README-OPTINS-COMPLETE-001` | Table des opt-ins du `README.md:114-128` incomplète (13 lignes / 15 paquets) : ajouter `forge-mvc-admin` (opt-in applicatif réel), et `forge-mvc-testing` avec mention « dev-only » (ADR-041) ou note d'exclusion explicite | 🟡 ✅ fait |
| `REFACTOR-CTRLCTX-DEAD-FIELDS-001` | Retirer les 2 champs morts `pk_col` et `rbac` de `_ControllerContext` (`cli/entities/crud/controller_builder.py:55,60`), lus par aucun `_render_*` (reliquat du refacto `REFACTOR-BUILD-CONTROLLER-001`). Vérifier iso-sortie via baseline | 🟡 ✅ fait |
| `GOV-ADR-STATUS-RESOLVE-001` | Trancher le statut d'ADR appliqués mais encore « Proposé » : ADR-030 (injection de routes, appliqué par `cli/public/*`) et ADR-041 (infra de test partagée, paquet `forge-mvc-testing` distribué) → « Acceptée » ou clarification | 🟡 ✅ fait |
| `ARCH-USER-CONTROLLER-REWRITE-ADR-001` | Ouvrir l'ADR distinct annoncé par ADR-030 : `cli/public/public_page.py:170` (`_ensure_controller_method`) **réécrit un contrôleur utilisateur in-place** (AST), ce qui touche le principe 4 (préserver le code utilisateur) et n'est couvert par aucun ADR accepté. Trancher : acter le comportement (idempotent, fail-safe) ou re-scoper `make:public-page` en mode « affichage » | 🟡 ✅ fait |
| `TEST-DOC-AS-TEST-ROTATION-002` | Poursuivre la rotation des tests doc-as-test : 45,6 % des tests sont `meta` (7 499), dont ~2 287 fonctions en pur grep de texte (cassent sur reformulation FR). Cibler en priorité `test_landing_*` et `test_docs_no_stale_versions_*` (les plus volatils) pour conversion en données structurées ou suppression, via `test_meta_tests_rotation_policy_001` | 🟡 ◑ 1re tranche |
| `TEST-DB-COLLECT-GUARD-001` | Fermer le skip silencieux des e2e MariaDB : `import mariadb` saute le module AVANT le check `FORGE_REQUIRE_DB` (`tests/test_e2e_mariadb.py:46-52`) ; le job `tests-db` resterait vert avec 0 test. En présence de `FORGE_REQUIRE_DB=1`, `pytest.fail` au lieu de `skip`, ou assertion « ≥ N tests `db` collectés ». Même esprit pour `importorskip` massif (169 fichiers) hors CI | 🟡 ✅ fait |
| `DOC-GUNICORN-RATELIMIT-001` | Documenter dans le guide de déploiement Gunicorn (chemin prod officiel) que le rate-limit login (`core/auth/rate_limit.py`) est **en mémoire par worker** : en multi-worker, `N×5` tentatives/fenêtre ; un backend partagé (DB/Redis) est requis pour un rate-limit effectif | 🟡 ✅ fait |
| `GOV-CLAUDE-MD-OPTINS-RESYNC-001` | Resync `CLAUDE.md` (§1, §3, §9 « 12 sous-dossiers/opt-ins » → 15 : admin, testing, qrcode ; tableau ADR §4 jusqu'à 050). Fichier protégé par le hook → mainteneur, au prochain tag. Complète `GOV-DOC-COHERENCE-001` | ⚪ ✅ fait |
| `TEST-META-CORE-NO-OPTIN-IMPORT-001` | Ajouter un garde-fou meta **global** : `rglob("core/**/*.py")` asserte l'absence de tout `import forge_mvc_` / `from forge_mvc_`. Aujourd'hui le découplage est propre mais verrouillé seulement par des tests ponctuels (audit.py, doctor.py) | ⚪ ✅ fait |
| `DOC-U2014-ROADMAP-RESIDUAL-001` | Corriger 2 cadratins de prose résiduels `docs/roadmap/forge-roadmap.md:170-171` (échappés au sweep racine, contredisent l'affirmation « 0 cadratin de prose » de la même page) | ⚪ ✅ fait |
| `CI-DEPAUDIT-DOC-LINK-001` | Corriger le lien doc cassé dans `.github/workflows/dependency-audit.yml` (commentaire) : `docs/release-policy.md` → `docs/release/release-policy.md` | ⚪ ✅ fait |
| `REFACTOR-BUILDERS-DECOMPOSE-002` | Finir la décomposition des builders CRUD : extraire `_render_list_context` + `_render_preamble` de `build_controller` (reste 317 l.), puis traiter `build_model` (254 l.), `build_form_view` (212 l.), `build_table_partial` (159 l.) sur le même modèle. Iso-sortie garantie par baseline | ⚪ ✅ fait |
| `SEC-MFA-DECRYPT-MSG-001` | Aligner `decrypt_totp_secret` (`packages/forge-mvc-mfa/forge_mvc_mfa/secret_crypto.py:186`) sur la discipline du reste du module : ne pas interpoler `{exc}` (message Fernet) dans `MfaSecretInvalidKey`. Pas de fuite réelle (`InvalidToken` vide), incohérence de style | ⚪ ✅ fait |
| `TEST-STRICT-MARKERS-001` | Ajouter `addopts = --strict-markers` à `pytest.ini` : une faute de frappe dans un futur `@pytest.mark.xxx` échouerait au lieu de passer silencieusement (les 3 marqueurs `meta`/`smoke`/`db` sont déjà déclarés) | ⚪ ✅ fait |
...
| `DEV-SERVER-AUTORELOAD-001` | **livré** | Superviseur d'autoreload (`cli.dev_reloader`) : `forge run` (dev, défaut) spawne `python app.py`, polling `stat()` sur `app.py`, `config.py`, `env/dev`, `mvc/**/*.{py,html,json,sql}` et `core/**/*.py`, redémarrage propre (`terminate` + `wait` + respawn) sur changement. Stdlib uniquement (pas de `watchfiles`/`watchdog`). Désactivable via `--no-reload` (chemin legacy `scripts/dev-server.sh`). Prod : inchangé (refus + message WSGI). |
...
| Ticket | État | Rôle |
|---|---|---|
| `ENTITIES-SESSIONS-MARIADB-LEAK-001` | **clos** (audit du 2026-07-27) | Le fichier `core/sessions/mariadb_store.py` n'existe plus : le store a été extrait vers l'opt-in `forge-mvc-sessions-db` sous le nom générique `DbSessionStore` (voir l'addendum de l'ADR-054, ticket `AUDIT-SESSION-STORE-EXTRACT-001`), et deux tests garantissent son absence du cœur. Hors docstrings du contrat `Dialect`, qui citent MariaDB et SQLite pour expliquer l'abstraction, le cœur ne référence plus aucun SGBD dans son code. Reste `GOV-SESSIONS-LEAK-CLOSE-001` pour les traces documentaires. |
| `GOV-SESSIONS-LEAK-CLOSE-001` | **sources closes**, republication à faire | Traces laissées par l'extraction ci-dessus. Corrigées : `docs/release/release-policy.md`, `SECURITY.md`, le « repli MariaDB intégré » annoncé par `core/database/connection.py` alors que `get_backend()` exige un opt-in, et l'ADR-002 qui reçoit un addendum de relecture. **Le diagnostic initial de ce ticket était faux sur la cause** : il visait `official-site/docs/forge/**`, qui n'est ni une source ni un intrant de publication mais un vestige local ignoré par git, d'avant l'ADR-045 (le site se construit depuis le `mkdocs.yml` racine, source `docs/`). Le site en ligne est bien trompeur, mesuré le 2026-07-27 (`https://forgemvc.com/docs/forge/reference/sessions/` enseigne encore `MariaDbSessionStore`), mais parce qu'il **n'a pas été republié** depuis l'extraction, pas à cause de ces fichiers. Reste donc deux gestes hors code : republier le site (`make docs-publish`, décision du mainteneur) et reprendre `CHARTE_DOC.md` § 5.1, que le hook interdit aux agents. Sévérité Moyenne : documentation publiée trompeuse, aucun effet runtime. |
| `OPTIN-DDL-DIALECT-AUDIT-001` | **clos** | Chantier terminé le 2026-07-27 : tout le DDL des opt-ins est rendu par le contrat `Dialect`, plus aucun paquet ne livre de SQL propre à MariaDB. Le garde-fou `tests/meta/test_optin_ddl_portability_ratchet_001.py` n'a plus aucune entrée. Rapport d'origine : [`docs/history/audits/optin-ddl-dialect-audit-001.md`](../history/audits/optin-ddl-dialect-audit-001.md). |
| `OPTIN-SQL-TYPE-BRANCHING-AUDIT-001` | **clos** | Audit livré puis corrigé le 2026-07-27 (voie « décider à partir du type Forge »). Le résolveur propage `forge_type` et expose `is_long_text` / `is_text_like` / `is_list_filterable`, que les générateurs consomment à la place des tests de préfixes SQL. Le CRUD généré retrouve sa recherche, ses `textarea` et ses libellés sur SQL Server ; les filtres de liste sont de nouveau acceptés sur SQLite et SQL Server ; deux défauts supplémentaires ont été trouvés en corrigeant (formulaires `date` et `datetime` refusés hors MariaDB). Rapport : [`docs/history/audits/optin-sql-type-branching-audit-001.md`](../history/audits/optin-sql-type-branching-audit-001.md). |
| `OPTIN-RUNTIME-PAGINATION-DIALECT-001` | **livré** | Cinq opt-ins bornaient leurs lectures en syntaxe MySQL dans le SQL qu'ils exécutent, hors des générateurs : `forge-mvc-admin`, `forge-mvc-audit`, `forge-mvc-iot`, `forge-mvc-mail` et `forge-mvc-notifications`. T-SQL ne connaissant pas `LIMIT`, ces lectures échouaient sur SQL Server, promu au niveau plein par l'ADR-084. Le contrat `Dialect` reçoit `limit_clause()`, distincte de `pagination_clause()` puisqu'elle ne porte qu'un paramètre. Rupture d'API publique dans `forge-mvc-iot` : les deux constantes `SELECT_IOT_EVENTS_*_SQL` deviennent des fonctions, une constante figée à l'import ne pouvant plus être correcte (même raison que la suppression des `CREATE_TABLE_SQL`). Vérifié sur un serveur SQL Server réel, les six requêtes produites s'exécutent. |
