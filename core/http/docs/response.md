...
- **Poser un cookie** : on écrit dans `response.headers["Set-Cookie"]`, par
  exemple via le cookie de session (voir [La session HTTP](../core-security/session.md)).
...
- [La session HTTP dans Forge](../core-security/session.md) : poser et retrouver la session via
