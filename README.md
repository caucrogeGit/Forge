# Forge — Framework MVC Python 1.0.0-beta.4
...
> **Forge 1.0.0-beta.4 — bêta publique.** Disponible sur [PyPI](https://pypi.org/project/forge-mvc/1.0.0b4/) sous `forge-mvc==1.0.0b4`.
> L'option `--pre` est nécessaire car `1.0.0b4` est une préversion bêta PEP 440.
...
```bash
git clone --branch v1.0.0-beta.4 --depth=1 https://github.com/caucrogeGit/Forge.git NomDuProjet
cd NomDuProjet
```
...
git init
git add -A
git commit -m "init: NomDuProjet — based on Forge 1.0.0-beta.4"
```
