...
conventions et réflexes utiles au développement d'une application Forge, sans
recopier la gouvernance interne du framework (charte intégrale, ADR, processus).
Versionné avec Forge ; même contenu pour `CLAUDE.md` et `AGENTS.md`.
"""
...
def render_app_briefing() -> str:
    """Retourne le briefing agent canonique pour une application Forge.

    Fonction (et non simple constante) pour laisser un point d'extension futur
    (injection de la version de Forge, du nom de projet, etc.) sans changer les
    appels de `forge new` et `forge agents:init`.
    """
    return APP_BRIEFING
