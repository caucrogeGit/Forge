...
def dry_run(meta: dict, *, public: bool = False) -> None:
    kind = meta.get("kind", "crud")
    if kind == "application":
        _dry_run_application(meta)
        return
    if kind == "skeleton":
        _dry_run_skeleton(meta)
        return
...
def _dry_run_skeleton(meta: dict) -> None:
...
def build(
    ...
    *,
    ...
    if kind == "application":
        ...
        return
    if kind == "skeleton":
        _build_skeleton(meta, force=force)
        return
    _build_crud(meta, init_db=init_db, force=force, public=public)


def _build_skeleton(meta: dict, *, force: bool = False) -> None:
    root = Path.cwd()
    ...
    if existing and force:
        ...
        force_clean_skeleton(meta, root)

    print(f"\nStarter : {meta['name']} (niveau {meta['number']})")

    next_step("Copie des fichiers...")
    ...
    _print_summary(meta, meta.get("open_url", "https://localhost:8000/"))
...
def _build_application(
    ...
    *,
    ...
    if not routes_py.exists():
        raise StarterBuildError("mvc/routes.py introuvable.")
    if meta.get("id") in ("utilisateurs-auth",):
