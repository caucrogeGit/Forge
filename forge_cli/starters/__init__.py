...
from forge_cli.starters.registry import all_starters, resolve, StarterNotFound, StarterUnavailable as StarterUnavailable
...
def cmd_starter_build(args: list[str]) -> None:
    ...
    if "--help" in args:
        print("Usage : forge starter:build <n|nom> [--dry-run] [--init-db] [--force] [--public]")
        print()
        print("Construit un starter Forge dans le projet courant.")
        print()
        print("Arguments :")
        print("  <n|nom>   numéro ou identifiant du starter (voir forge starter:list)")
        print()
        print("Options :")
        print("  --dry-run    affiche ce qui serait fait sans rien écrire")
        print("  --init-db    initialise la base (forge db:init) avant le build")
        print("  --force      écrase les fichiers existants")
        print("  --public     génère des routes publiques (si applicable)")
        raise SystemExit(0)
