"""Smoke test du paquet forge-mvc-mssql (OPTIN-SMOKE-TESTS-001, ADR-040).

Vérifie l'import, l'API publique, py.typed, le contrat de backend et l'entry
point, sans connexion (pyodbc importé paresseusement). Skip si non installé.
"""
...
def test_backend_respecte_le_contrat() -> None:
    ...
    backend = mod.MSSQLBackend()
    assert backend.name == "mssql"
...
def test_entry_point_db_backend_declare() -> None:
    ...
    assert "mssql" in names, (
        "forge-mvc-mssql doit déclarer l'entry point forge_mvc.db_backend "
        f"nommé 'mssql' (trouvés : {sorted(names)})."
    )
