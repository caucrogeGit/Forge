"""Smoke test du paquet forge-mvc-sqlite (OPTIN-SMOKE-TESTS-001, ADR-040).
...
def test_entry_point_db_backend_declare() -> None:
    ...
    assert "sqlite" in names, (
        "forge-mvc-sqlite doit déclarer l'entry point forge_mvc.db_backend "
        f"nommé 'sqlite' (trouvés : {sorted(names)})."
    )
