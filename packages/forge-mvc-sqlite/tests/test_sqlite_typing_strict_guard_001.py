"""Garde-fou SQLITE-TYPING-STRICT-GUARD-001 : forge-mvc-sqlite strict + typé.
...
import forge_mvc_sqlite  # noqa: E402

PKG_ROOT = Path(forge_mvc_sqlite.__file__).resolve().parent
...
def test_tout_forge_mvc_sqlite_est_pyright_strict() -> None:
    missing: list[str] = []
...
def test_py_typed_present() -> None:
    assert (PKG_ROOT / "py.typed").is_file(), (
        "forge_mvc_sqlite doit embarquer py.typed (PEP 561)"
    )
