# pyright: strict
"""Commande ``forge settings:init`` — SETTINGS-INIT-COMMAND-001.

Copie la migration SQL embarquée (`forge_mvc_settings/migrations/`) vers le
dossier `mvc/migrations/` du projet, sans aucune exécution SQL ni connexion
MariaDB. On prépare le fichier, puis on suggère `forge migration:apply`.
...
def iter_settings_migration_resources() -> Iterator[tuple[str, bytes]]:
    ...
    anchor = resources.files("forge_mvc_settings") / "migrations"
...
def init_settings_migrations(project_root: Path) -> int:
    ...
    for name, content in iter_settings_migration_resources():
    ...
    for name in copied:
        print(f"{STATUS_OK} Migration Settings copiée : mvc/migrations/{name}")
    for name in skipped_identical:
        print(f"{STATUS_OK} Migration Settings déjà présente (identique) : mvc/migrations/{name}")
