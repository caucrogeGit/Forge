"""Smoke test du paquet forge-mvc-settings (OPTIN-SMOKE-TESTS-001).
...
ET en autonome. Skip propre si le paquet n'est pas installé.
"""
...
def test_ships_migration() -> None:
    assert mod.__file__ is not None
    migrations = Path(mod.__file__).parent / "migrations"
    sql = list(migrations.glob("*.sql"))
    assert sql, f"{MODULE} doit embarquer au moins une migration .sql"
