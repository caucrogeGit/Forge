...
|--------|--------------------|
| 1, [Premier paramètre](settings-welcome.md) | Créer la table (`settings:init` puis `migration:apply`), écrire (`set_setting`) et relire (`get_setting`). |
| 2, [Écrire et lire](settings-set-get.md) | Mettre à jour une valeur (upsert) ; lire avec une valeur par défaut. |

Vous savez créer la table des paramètres, écrire une valeur et la relire.
