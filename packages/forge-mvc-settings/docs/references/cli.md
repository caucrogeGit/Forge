...
La table `app_settings` n'est pas créée automatiquement : c'est une écriture en base, donc elle reste explicite et visible (principe 3, SQL visible).
`forge settings:init` copie la migration SQL embarquée dans le paquet vers le dossier `mvc/migrations/` du projet.
Aucune connexion MariaDB, aucune exécution SQL à cette étape : on prépare seulement le fichier.
...
- un fichier présent mais au contenu différent déclenche un `[WARN]` et n'est pas écrasé.

Si le dossier `mvc/` est absent, la commande s'arrête avec un message clair (`[ERREUR]`) : elle doit être lancée à la racine d'un projet Forge.
