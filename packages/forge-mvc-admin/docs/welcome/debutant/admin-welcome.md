...
!!! note "Prérequis : installer l'opt-in"
    ...
    ```bash
    pip install --pre forge-mvc-admin    # installe le paquet
    forge opt-in:enable admin          # le branche au projet
    ```

    Sans le paquet, l'application refuse de démarrer sur un `ModuleNotFoundError` au chargement des routes.

    `forge opt-in:install admin` **affiche** la commande d'installation adaptée à votre environnement, pipx compris ; il n'installe rien lui-même (ADR-016).
