# pyright: strict
"""Commande ``forge audit:init`` — AUDIT-INIT-COMMAND-001.

Copie la migration SQL embarquée (`forge_mvc_audit/migrations/`) vers le dossier
`mvc/migrations/` du projet, sans aucune exécution SQL ni connexion MariaDB. On
prépare le fichier, puis on suggère `forge migration:apply`.
...
__all__ = [
    ...
    "STATUS_WARN",
    "STATUS_ERROR",
    "iter_audit_migration_resources",
    "init_audit_migrations",
    "main",
]
...
def iter_audit_migration_resources() -> Iterator[tuple[str, bytes]]:
    """Itère ``(filename, content_bytes)`` pour chaque ``.sql`` embarqué."""
    anchor = resources.files("forge_mvc_audit") / "migrations"
    for entry in sorted(anchor.iterdir(), key=lambda e: e.name):
        if entry.name.endswith(".sql"):
            yield entry.name, entry.read_bytes()


def init_audit_migrations(project_root: Path) -> int:
    """Copie les migrations Audit vers ``<project_root>/mvc/migrations/``.
    ...
    for name, content in iter_audit_migration_resources():
    ...
    for name in copied:
        print(f"{STATUS_OK} Migration Audit copiée : mvc/migrations/{name}")
    for name in skipped_identical:
        print(f"{STATUS_OK} Migration Audit déjà présente (identique) : mvc/migrations/{name}")
...
def main(args: list[str] | None = None) -> int:
    """Point d'entrée appelé par ``forge.py`` pour ``forge audit:init``."""
    return init_audit_migrations(Path.cwd())
