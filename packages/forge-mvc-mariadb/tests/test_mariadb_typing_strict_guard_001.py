"""Garde-fou MARIADB-TYPING-STRICT-GUARD-001 : forge-mvc-mariadb strict + typé.
...
import forge_mvc_mariadb  # noqa: E402

PKG_ROOT = Path(forge_mvc_mariadb.__file__).resolve().parent
...
def test_tout_forge_mvc_mariadb_est_pyright_strict() -> None:
    missing: list[str] = []
...
def test_py_typed_present() -> None:
    assert (PKG_ROOT / "py.typed").is_file(), (
        "forge_mvc_mariadb doit embarquer py.typed (PEP 561)"
    )
