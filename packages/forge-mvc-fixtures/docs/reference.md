...
??? note "10. Fixtures callable (hooks Python, ADR-078)"
    ...
    `fixtures:purge` démonte dans l'ordre **inverse exact** du chargement (le même graphe topologique renversé, `.sql` et callable) : les enfants avant les parents, si bien que `fixtures:purge --run` puis `fixtures:load --run` reconstruit un état propre sans violer de clé étrangère, de façon rejouable.
