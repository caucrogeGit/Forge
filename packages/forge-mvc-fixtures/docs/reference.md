...
??? note "10. Fixtures callable (hooks Python, ADR-078)"
    ...
    `fixtures:purge` démonte dans l'ordre **inverse exact** du chargement (le même graphe topologique renversé, `.sql` et callable), et encadre l'opération par la **désactivation des contraintes FK** du dialecte (`SET FOREIGN_KEY_CHECKS`, `PRAGMA foreign_keys`...), robuste même pour un callable peuplant plusieurs tables liées. Si bien que `fixtures:purge --run` puis `fixtures:load --run` reconstruit un état propre sans erreur de clé étrangère, de façon rejouable.
