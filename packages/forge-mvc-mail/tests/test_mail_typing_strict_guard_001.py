"""Garde-fou MAIL-TYPING-STRICT-GUARD-001 : forge-mvc-mail reste strict + typé.

Troisième opt-in passé en pyright strict (suite du cliquet ADR-036, après le
cœur, ``forge-mvc-files`` et ``forge-mvc-images``). Ce test verrouille deux
acquis :
...
PKG_ROOT = Path(forge_mvc_mail.__file__).resolve().parent
...
def test_tout_forge_mvc_mail_est_pyright_strict():
    missing: list[str] = []
    ...
    assert missing == [], (
        f"Fichiers de forge_mvc_mail/ sans `{MARKER}` : {missing}. "
        "Tout module du paquet doit être strict (cliquet typage opt-ins)."
    )


def test_forge_mvc_mail_expose_py_typed():
    marker = PKG_ROOT / "py.typed"
    assert marker.exists(), (
        "forge_mvc_mail/py.typed attendu (PEP 561) : sans lui, le paquet est "
