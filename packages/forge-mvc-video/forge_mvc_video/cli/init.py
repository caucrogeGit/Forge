...
``forge_mvc_video/migrations/`` vers le dossier ``mvc/migrations/`` du projet
utilisateur. **Aucune exécution SQL**, **aucune connexion MariaDB** : on
prépare seulement les fichiers, puis on suggère ``forge migration:apply``.

Volontairement scoped (mirror ``iot:init``) :

- idempotente (rejouable sans erreur si la migration est déjà copiée à
  l'identique) ;
- jamais d'écrasement silencieux : un fichier déjà présent avec un contenu
  différent provoque un ``WARN`` et n'est pas écrasé (charte §9) ;
- pas de choix interactif, pas de rollback.

Lecture des ressources via ``importlib.resources.files("forge_mvc_video")`` —
fonctionne identiquement en install éditable, en wheel et en sdist.
"""
...
def init_video_migrations(project_root: Path) -> int:
    ...
    -------
    0
        Succès (y compris idempotent : fichiers déjà présents identiques).
    1
    ...
    for name, content in iter_video_migration_resources():
    ...
    for name in copied:
        print(f"{STATUS_OK} Migration Vidéo copiée : mvc/migrations/{name}")
    for name in skipped_identical:
        print(
            f"{STATUS_OK} Migration Vidéo déjà présente (identique) : "
