# pyright: strict
"""Commande ``forge images:init`` — OPTIN-IMAGES-INIT-001.

Copie la migration SQL embarquée (`forge_mvc_images/migrations/`) vers le
dossier `mvc/migrations/` du projet, sans aucune exécution SQL ni connexion
base. On prépare le fichier, puis on suggère `forge migration:apply`.

La table `media` sert la couche médias applicative (repository, galerie,
couverture) ; la couche traitement d'image n'en dépend pas.
...
def iter_images_migration_resources() -> Iterator[tuple[str, bytes]]:
    ...
    anchor = resources.files("forge_mvc_images") / "migrations"
...
def init_images_migrations(project_root: Path) -> int:
    ...
    for name, content in iter_images_migration_resources():
    ...
    for name in copied:
        print(f"{STATUS_OK} Migration Images copiée : mvc/migrations/{name}")
    for name in skipped_identical:
        print(f"{STATUS_OK} Migration Images déjà présente (identique) : mvc/migrations/{name}")
