"""Smoke test du paquet forge-mvc-sessions-db (ADR-040).
...
ET en autonome (`cd packages/forge-mvc-sessions-db && pytest`). Skip propre si
le paquet n'est pas installé (convention `pytest.importorskip` du projet).
"""
...
def test_store_sql_is_portable() -> None:
    """Aucune fonction date propriétaire dans les requêtes SQL (ADR-054)."""
    from forge_mvc_sessions_db import store

    sqls = [
        store._SQL_INSERT,
        store._SQL_SELECT,
        store._SQL_UPDATE,
        store._SQL_UPDATE_EXPIRY,
        store._SQL_DELETE,
        store._SQL_CLEANUP,
    ]
    for sql in sqls:
        assert "NOW()" not in sql, f"NOW() propriétaire dans : {sql}"
        assert "GETDATE()" not in sql, f"GETDATE() propriétaire dans : {sql}"
        assert "datetime('now')" not in sql, f"datetime('now') propriétaire dans : {sql}"
