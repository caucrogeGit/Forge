# pyright: strict
"""Commande ``forge sessions:init`` — provisionne le schéma (retour terrain 016 F34 ; ADR-071).

Copie la migration SQL embarquée (`forge_mvc_sessions_db/migrations/`) vers le
...
def iter_sessions_migration_resources() -> Iterator[tuple[str, bytes]]:
    ...
    anchor = resources.files("forge_mvc_sessions_db") / "migrations"
...
def init_sessions_migrations(project_root: Path) -> int:
    ...
    for name, content in iter_sessions_migration_resources():
    ...
    for name in copied:
        print(f"{STATUS_OK} Migration Sessions copiée : mvc/migrations/{name}")
    for name in skipped_identical:
        print(f"{STATUS_OK} Migration Sessions déjà présente (identique) : mvc/migrations/{name}")
