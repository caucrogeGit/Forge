...
Récapitulatif du **niveau avancé** de la progression *Welcome RBAC*.
Ce niveau **relie** rôles et permissions aux utilisateurs réels.
...
Vous maîtrisez le RBAC de bout en bout : déclarer, vérifier, appliquer, relier aux utilisateurs.

## Et ensuite

La progression *Welcome RBAC* est terminée.
En production : déclarez le contrat `mvc/security/rbac.json`, persistez rôles et associations (`rbac`, `user_roles`), gardez les routes (`require_*`) et adaptez l'UI (`can()`).
