...
`normalize_status_name` le met en forme (« En Revue » → `en_revue`) ; `validate_status_name` refuse les noms invalides.
...
- Normaliser **avant** de comparer évite les faux négatifs (« En Revue » vs `en_revue`).
...
[Retrouver un statut](workflow-find.md)
