"""Garde-fou WORKFLOW-TYPING-STRICT-GUARD-001 : forge-mvc-workflow strict + typé.

Neuvième opt-in passé en pyright strict (suite du cliquet ADR-036). Ce test
...
PKG_ROOT = Path(forge_mvc_workflow.__file__).resolve().parent
...
def test_tout_forge_mvc_workflow_est_pyright_strict():
    missing: list[str] = []
    ...
    assert missing == [], (
        f"Fichiers de forge_mvc_workflow/ sans `{MARKER}` : {missing}. "
        "Tout module du paquet doit être strict (cliquet typage opt-ins)."
    )


def test_forge_mvc_workflow_expose_py_typed():
    marker = PKG_ROOT / "py.typed"
    assert marker.exists(), (
        "forge_mvc_workflow/py.typed attendu (PEP 561) : sans lui, le paquet est "
