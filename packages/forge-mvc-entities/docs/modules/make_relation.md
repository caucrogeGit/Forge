...
Pour une relation `many_to_one`, elle **injecte aussi** la clé étrangère comme champ de première classe dans le JSON de l'entité source (type `foreign_key`, ADR-069), de façon chirurgicale et idempotente (écriture annoncée `[MODIFIE]`, préserve les autres champs).
La colonne FK vient alors de l'entité ; `relations.sql` ne pose que la contrainte.
