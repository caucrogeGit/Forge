...
def _render_model_query(
    ...
    lines.extend([
        "",
        "",
        "# Plafond d'export. Modifiable par l'application : c'est une décision",
        "# d'exploitation, qui dépend de la taille des lignes et de la mémoire",
        "# du serveur, pas une constante du framework.",
        "_EXPORT_LIMIT = 1000",
        "",
        "",
        f'def find_{plural}_for_export(q: str | None = None, sort: str | None = None, direction: str = "asc", filters: dict[str, Any] | None = None) -> tuple[list[dict[str, Any]], bool]:',
        '    """Lignes à exporter, et un drapeau disant si le plafond a coupé.',
        "",
        "    IMPEXP-FILTERED-EXPORT-001 : la fonction rendait la liste seule, et",
        "    l'export était donc tronqué EN SILENCE au delà du plafond. Un",
        "    utilisateur qui filtrait trois mille lignes en recevait mille, dans",
        "    un fichier impossible à distinguer d'un export complet.",
        "",
        "    Une ligne de plus que le plafond est demandée : c'est la seule façon",
        "    de savoir qu'il en restait, sans payer un COUNT sur la même requête.",
        '    """',
        f"    rows = find_{plural}_paginated(",
        "        q=q, sort=sort, direction=direction,",
        "        limit=_EXPORT_LIMIT + 1, offset=0, filters=filters,",
        "    )",
        "    truncated = len(rows) > _EXPORT_LIMIT",
        "    return (rows[:_EXPORT_LIMIT], truncated)",
        "",
    ])
    return lines
