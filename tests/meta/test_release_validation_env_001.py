...
Vérifie que docs/release/release-local.md contient les éléments d'un environnement
...
def _text() -> str:
    if not RELEASE_LOCAL.exists():
        pytest.skip("docs/release/release-local.md n'existe pas")
...
class TestReleaseValidationEnvSection:
    ...
    def test_section_environnement_presente(self):
        assert "Environnement de validation release" in _text(), (
            "docs/release/release-local.md doit contenir 'Environnement de validation release' "
            "(RELEASE-VALIDATION-ENV-LOCK-001)"
        )

    def test_pytest_mentionne(self):
        assert "pytest" in _text(), (
            "docs/release/release-local.md doit mentionner 'pytest' dans la procédure de validation"
        )

    def test_mkdocs_strict_mentionne(self):
        assert "mkdocs build --strict" in _text(), (
            "docs/release/release-local.md doit mentionner 'mkdocs build --strict'"
        )

    def test_python_build_mentionne(self):
        assert "python -m build" in _text(), (
            "docs/release/release-local.md doit mentionner 'python -m build'"
        )

    def test_twine_check_mentionne(self):
        assert "twine check dist/*" in _text(), (
            "docs/release/release-local.md doit mentionner 'twine check dist/*' "
    ...
    def test_ne_publie_pas(self):
        ...
        assert "ne publie" in text or "ne publie rien" in text, (
            "docs/release/release-local.md doit indiquer que la procédure ne publie rien"
        )

    def test_release_check_script_reference_ticket_22(self):
        text = _text()
        assert "scripts/release_check.sh" in text, (
            "docs/release/release-local.md doit référencer 'scripts/release_check.sh'"
        )
        assert "RELEASE-CHECK-SCRIPT-001" in text or "2.2" in text, (
            "docs/release/release-local.md doit indiquer que le script relève du ticket 2.2"
        )
