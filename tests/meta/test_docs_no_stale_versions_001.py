...
class TestForgePyVersionConsistency:
    ...
    def test_forge_version_current(self):
        ...
        assert match.group(1) == "1.0.0b2", (
            f"forge.py _FORGE_VERSION = '{match.group(1)}', attendu '1.0.0b2'."
        )
    ...
    def test_forge_default_ref_current(self):
        ...
        assert match.group(1) == "v1.0.0-beta.2", (
            f"forge.py _FORGE_DEFAULT_REF = '{match.group(1)}', attendu 'v1.0.0-beta.2'."
        )
