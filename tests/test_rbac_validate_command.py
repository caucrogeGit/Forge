...
def _run(args: list[str], *, cwd: Path | None = None) -> tuple[str, str, int]:
    ...
    with patch("sys.stdout", out), patch("sys.stderr", err):
        try:
            if cwd is not None:
                with patch("forge_cli.rbac_validate.Path.cwd", return_value=cwd):
...
def _run_in(tmp_path: Path, args: list[str]) -> tuple[str, str, int]:
    ...
    with patch("sys.stdout", out), patch("sys.stderr", err):
        try:
            with patch("forge_cli.rbac_validate.Path.cwd", return_value=tmp_path):
                rbac_validate_main(args)
        except SystemExit as exc:
            code = int(exc.code) if exc.code is not None else 0
    return out.getvalue(), err.getvalue(), code
...
def test_module_importable():
    from forge_cli import rbac_validate  # noqa: F401
...
def test_forge_py_dispatche_rbac_validate(tmp_path, capsys):
    ...
    with patch("sys.argv", ["forge", "rbac:validate"]), \
         patch("forge_cli.rbac_validate.Path.cwd", return_value=tmp_path):
        try:
            forge.main()
        except SystemExit:
            pass
