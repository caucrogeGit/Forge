...
Vérifie le contrat pédagogique du starter d'entrée Forge IoT :
  ...
  2. ``starter.json`` est valide et déclare le nom public
     « Bonjour IoT », `requires_db: false`, alias `welcome-optin-iot`/`iot`.
...
class TestStarterJsonContract:
    ...
    def test_id(self):
        assert self.meta["id"] == "welcome-optin-iot"
    ...
    def test_home_route(self):
        assert self.meta["home_route"] == "/welcome-optin-iot"

    def test_routes_marker(self):
        assert self.meta["routes_marker"] == "welcome-optin-iot"
    ...
    @pytest.mark.parametrize(
        "alias",
        ["welcome-optin-iot", "welcome_optin_iot", "bonjour-iot", "bonjour_iot", "iot", "12"],
    )
    def test_alias_resolvable(self, alias):
        resolved = resolve(alias)
        assert resolved["id"] == "welcome-optin-iot"
...
class TestRoutesSnippet:
    ...
    def test_has_start_end_markers(self):
        assert "# forge-starter:welcome-optin-iot:start" in self.text
        assert "# forge-starter:welcome-optin-iot:end" in self.text

    @pytest.mark.parametrize(
        "url",
        [
            "/welcome-optin-iot",
            "/welcome-optin-iot/inspect",
            "/welcome-optin-iot/events",
            "/welcome-optin-iot/device/{site}/{device_id}",
        ],
    )
    ...
    def test_imports_controller(self):
        assert (
            "from mvc.controllers.welcome_optin_iot_controller "
...
def controller_module():
    ...
    try:
        if "welcome_optin_iot_controller" in sys.modules:
            del sys.modules["welcome_optin_iot_controller"]
        module = importlib.import_module("welcome_optin_iot_controller")
        yield module
    finally:
        sys.path.remove(str(controllers_dir))
        sys.modules.pop("welcome_optin_iot_controller", None)
...
class TestControllerIndex:
    def test_returns_bonjour_forge_iot(self, controller_module):
        ...
        resp = ctrl.index(FakeRequest("GET", "/welcome-optin-iot"))
...
class TestControllerInspect:
    def test_returns_json_with_default_config(self, controller_module):
        ctrl = controller_module.WelcomeIotController
        resp = ctrl.inspect(FakeRequest("GET", "/welcome-optin-iot/inspect"))
        assert resp.status == 200
...
class TestControllerEventsFallback:
    ...
    def test_events_returns_pedagogical_503_on_error(self, controller_module):
        with patch.object(
            ...
            ctrl = controller_module.WelcomeIotController
            resp = ctrl.events(FakeRequest("GET", "/welcome-optin-iot/events"))
...
class TestControllerFindByDevice:
    def test_returns_pedagogical_503_on_error(self, controller_module):
        with patch.object(
            ...
            ctrl = controller_module.WelcomeIotController
            req = FakeRequest("GET", "/welcome-optin-iot/device/atelier/esp32-001")
...
class TestDocumentation:
    ...
    def test_starters_index_lists_welcome_optin_iot(self):
        text = _STARTERS_INDEX.read_text(encoding="utf-8")
        assert "welcome-optin-iot" in text
        assert "Bonjour IoT" in text

    def test_mkdocs_nav_lists_welcome_optin_iot(self):
        text = _MKDOCS_YML.read_text(encoding="utf-8")
        assert "starters/optin-iot/welcome-optin-iot.md" in text
