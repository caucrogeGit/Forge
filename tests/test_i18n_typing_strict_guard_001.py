"""Garde-fou I18N-TYPING-STRICT-GUARD-001 : forge-mvc-i18n reste strict + typé.

Cinquième opt-in passé en pyright strict (suite du cliquet ADR-036, après le
cœur, ``forge-mvc-files``, ``forge-mvc-images``, ``forge-mvc-mail`` et
``forge-mvc-pivot``). Ce test verrouille deux acquis :
...
import forge_mvc_i18n

PKG_ROOT = Path(forge_mvc_i18n.__file__).resolve().parent
...
def test_tout_forge_mvc_i18n_est_pyright_strict():
    missing: list[str] = []
    ...
    assert missing == [], (
        f"Fichiers de forge_mvc_i18n/ sans `{MARKER}` : {missing}. "
        "Tout module du paquet doit être strict (cliquet typage opt-ins)."
    )


def test_forge_mvc_i18n_expose_py_typed():
    marker = PKG_ROOT / "py.typed"
    assert marker.exists(), (
        "forge_mvc_i18n/py.typed attendu (PEP 561) : sans lui, le paquet est "
