...
class TestLoginForm:
    ...
    def test_cree_une_session(self):
        ...
        sid = next((p.split("=", 1)[1] for p in cookie.split(";") if "__Host-session_id=" in p), None)
        assert sid is not None and _s.get_session(sid) is not None
...
class TestLoginSuccess:
    ...
    def test_redirige_vers_accueil(self, _cp, _gub):
    ...
    @patch("mvc.controllers.auth_controller.get_user_by_login", return_value=_GOOD_USER)
    @patch("mvc.controllers.auth_controller._check_password", return_value=True)
    def test_rotation_session_id(self, _cp, _gub):
        sid  = _s.create_session()
        sess = _s.get_session(sid)
        req  = FakeRequest("POST", "/login",
                           body={"login": "admin", "password": "secret",
    ...
    @patch("mvc.controllers.auth_controller.get_user_by_login", return_value=_GOOD_USER)
    @patch("mvc.controllers.auth_controller._check_password", return_value=True)
    def test_nouveau_cookie_dans_la_reponse(self, _cp, _gub):
        sid  = _s.create_session()
        sess = _s.get_session(sid)
        req  = FakeRequest("POST", "/login",
                           body={"login": "admin", "password": "secret",
...
class TestLoginFailure:
    ...
    def test_mauvais_mot_de_passe_retourne_200(self, _cp, _gub):
        sid  = _s.create_session()
        sess = _s.get_session(sid)
        req  = FakeRequest("POST", "/login",
...
class TestLogout:
    def _authentified_session(self):
        ...
        sid  = _s.create_session()
        new_sid = authenticate_session(sid, _GOOD_USER)
