"""OPTIN-DDL-SESSIONS-DB-001 (PostgreSQL) — la migration rendue s'exécute.
...
`INDEX` en ligne : il était rejeté par PostgreSQL dès la première instruction.

Marqué `db` + `db_pg` : sauté sans serveur, requis en CI via
FORGE_REQUIRE_DB_PG=1.
"""
...
def test_migration_rendue_sexecute_et_la_table_est_utilisable(real_pg_db: None) -> None:
