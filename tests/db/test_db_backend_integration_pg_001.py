"""CI-DB-POSTGRES-001 (ADR-084) — intégration de la couche DB sur un vrai PostgreSQL.

Exerce la VRAIE couche `core.database.db` contre un serveur PostgreSQL réel,
miroir de test_db_integration_001 (MariaDB) : aller-retour insert/fetch avec
identité d'insertion (lastrowid sous garde savepoint, PG-INSERT-IDENTITY-001),
rowcount des UPDATE/DELETE, liaison des paramètres (anti-injection), commit et
rollback d'une transaction explicite, contrainte de clé étrangère.
...
def test_insert_fetch_roundtrip_et_lastrowid(real_pg_db: None) -> None:
    table = _table("forge_it_pg_users")
    db.execute(f"CREATE TABLE {table} (id BIGSERIAL PRIMARY KEY, name VARCHAR(100) NOT NULL)")
    try:
        # PG-INSERT-IDENTITY-001 : db.insert retourne l'identité réelle
        # (lastval() sous savepoint), prouvé face au moteur.
...
def test_execute_update_and_delete_return_rowcount(real_pg_db: None) -> None:
    table = _table("forge_it_pg_items")
...
def test_parameters_bind_and_block_injection(real_pg_db: None) -> None:
    ...
    table = _table("forge_it_pg_inj")
    db.execute(f"CREATE TABLE {table} (id BIGSERIAL PRIMARY KEY, name VARCHAR(100))")
    try:
...
def test_lastrowid_sans_sequence_ne_perd_pas_l_insert(real_pg_db: None) -> None:
    # Garde savepoint (PG-INSERT-IDENTITY-001) : un INSERT via db.insert dans
    # une table SANS colonne à séquence retourne None (lastval() indéfini) mais
    # l'insertion est bien commitée — la transaction n'est pas avortée.
    table = _table("forge_it_pg_noseq")
    db.execute(f"CREATE TABLE {table} (id INT PRIMARY KEY, label VARCHAR(50))")
    try:
...
def test_transaction_commit_persists(real_pg_db: None) -> None:
    table = _table("forge_it_pg_tx")
...
def test_transaction_rollback_discards(real_pg_db: None) -> None:
    table = _table("forge_it_pg_rb")
    db.execute(f"CREATE TABLE {table} (id INT PRIMARY KEY)")
    try:
