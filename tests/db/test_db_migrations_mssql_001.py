"""MSSQL-MIGRATIONS-INTEGRATION-001 (ADR-084) — runner de migrations sur un vrai SQL Server.
...
@pytest.fixture()
def mssql_registry(real_mssql_db: None) -> Any:
...
def _table() -> str:
    return f"forge_it_ms_widgets_{uuid.uuid4().hex[:10]}"
...
def test_apply_creates_table_and_records_migration(mssql_registry: None, tmp_path: Path) -> None:
    table = _table()
...
def test_apply_is_idempotent(mssql_registry: None, tmp_path: Path) -> None:
    table = _table()
    (tmp_path / "20260101000000_create_widgets.sql").write_text(_widgets_sql(table), encoding="utf-8")
    try:
        _apply(tmp_path)
...
def test_apply_dry_run_writes_nothing(mssql_registry: None, tmp_path: Path) -> None:
    table = _table()
    (tmp_path / "20260101000000_create_widgets.sql").write_text(_widgets_sql(table), encoding="utf-8")
...
def test_apply_refuses_changed_migration(mssql_registry: None, tmp_path: Path) -> None:
    table = _table()
    ...
    try:
        ...
        path.write_text(_widgets_sql(table).replace("NVARCHAR(50)", "NVARCHAR(99)"), encoding="utf-8")
...
def test_apply_rolls_back_on_sql_failure(mssql_registry: None, tmp_path: Path) -> None:
...
def test_introspection_reelle_du_schema(mssql_registry: None, tmp_path: Path) -> None:
    """load_table_columns lit le vrai INFORMATION_SCHEMA SQL Server."""
    table = _table()
    ...
    try:
        ...
        assert by_name["id"].auto_increment is True, "IDENTITY doit être vu comme auto"
